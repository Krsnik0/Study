...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    Inputs:
    - X: Input data. shape (N, D). 각각의 X[i]가 하나의 training sample이며 
    총 N개의 sample이 input으로 주어짐.
    ...
    firstLayer = torch.add(torch.mm(X, W1), b1)
    h = torch.max(0 * firstLayer, firstLayer)
    scores = torch.add(torch.mm(h, W2), b2)
    ...
    #############################################################################
    gradients = softmax
    gradients[range(N), y] -= 1
    gradients /= N
    grads['W2'] = torch.mm(h.t(), gradients)
    grads['b2'] = torch.sum(gradients, dim = 0, keepdim=True)

    grad_h = torch.mm(gradients, W2.t())

    grad_h[h<=0] = 0
    grads['W1'] = torch.mm(X.t(), grad_h)
    grads['b1'] = torch.sum(grad_h, dim=0, keepdim=True)
  ...
  def train(self, X, y,
    ...
    for it in range(num_iters):
      ...
      self.params["b1"] -= grads["b1"].reshape(self.params["b1"].shape)*learning_rate
      self.params["b2"] -= grads["b2"].reshape(self.params["b2"].shape)*learning_rate
      #print("loss2: ", loss)
      pass
      #########################################################################
      #                             END OF YOUR CODE                          #
